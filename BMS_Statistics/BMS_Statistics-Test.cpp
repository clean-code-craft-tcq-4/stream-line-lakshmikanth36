...
TEST_CASE("Get BMS min and max temp and soc data") {
    ...
    REQUIRE(BatteryTempdata[MAX_RECEIVE_DATA-1] == SortedTempdata[MAX_RECEIVE_DATA-1]);
    
    REQUIRE(BatterySOCdata[0] == SortedSOCdata[0]);
    REQUIRE(BatterySOCdata[MAX_RECEIVE_DATA-1] == SortedSOCdata[MAX_RECEIVE_DATA-1]);
}
